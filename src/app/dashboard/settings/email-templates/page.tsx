...
export default function EmailTemplatesPage() {
  ...
  const handleSaveTemplate = async (template: EmailTemplate) => {
    try {
      ...
      toast({ title: "Template saved", description: "Email template saved successfully." });
    } catch (error) {
  ...
  const handleSendTest = async (email: string, variables: Record<string, string>) => {
    ...
    try {
      ...
      toast({ title: "Test email sent", description: "A test email was sent successfully." });
    } catch (error) {
    ...
    return (
      <div className="max-w-7xl mx-auto space-y-8">
        <div className="flex justify-between items-center">
          ...
          <Button onClick={refreshTemplates} aria-label="Refresh email templates" variant="outline" className="transition-colors hover:bg-gray-100 dark:hover:bg-gray-800 focus-visible:ring-2 focus-visible:ring-offset-2 focus-visible:ring-primary">
              ...
              <div className="space-y-2">
                {templates.map((template) => (
                  <div
                    ...
                    tabIndex={0}
                    onKeyDown={e => { if (e.key === 'Enter' || e.key === ' ') setActiveTemplate(template); }}
                  >
                    <div className="flex justify-between items-start mb-1">
                      <h3 className="font-medium text-sm">
                        {template.name}
                      </h3>
                      <Badge className={`${getTemplateBadgeColor(template.id)} text-xs font-normal transition-colors hover:brightness-110 focus-visible:ring-2 focus-visible:ring-offset-2 focus-visible:ring-primary`}>
